import og_dspy
from og_dspy.primitives.program import Module
...
class ChainOfThought(Module):
    def __init__(self, signature, rationale_type=None, activated=True, **config):
        ...
        *_keys, last_key = signature.output_fields.keys()
        ...
        rationale_type = rationale_type or og_dspy.OutputField(prefix=prefix, desc=desc)
        # Add "rationale" field to the output signature.
        extended_signature = signature.prepend("rationale", rationale_type, type_=str)
        self._predict = og_dspy.Predict(extended_signature, **config)
