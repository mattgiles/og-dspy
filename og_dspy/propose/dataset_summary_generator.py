...
class DatasetDescriptorWithPriorObservations(og_dspy.Signature):
    ...
    observations = og_dspy.OutputField(desc="Somethings that holds true for most or all of the data you observed or COMPLETE if you have nothing to add")
...
def create_dataset_summary(trainset, view_data_batch_size, prompt_model, log_file=None):
    ...
    with og_dspy.settings.context(lm=prompt_model):
        observation = og_dspy.Predict(DatasetDescriptor, n=1, temperature=1.0)(examples=order_input_keys_in_string(trainset[0:upper_lim].__repr__()))
    ...
    try:
        ...
        for b in range(view_data_batch_size, len(trainset), view_data_batch_size):
            ...
            with og_dspy.settings.context(lm=prompt_model):
                output = og_dspy.Predict(DatasetDescriptorWithPriorObservations, n=1, temperature=1.0)(prior_observations=observations, examples=order_input_keys_in_string(trainset[b:upper_lim].__repr__()))
