...
Your task is to propose a new improved instruction and prefix for the output field that will lead a good language model to perform the task well. Don't be afraid to be creative.""")
        example_instructions = og_dspy.InputField(format=dsp.passages2text, desc="Example instruction(s) for this task.")
...
Your task is to propose a new improved instruction and prefix for the output field that will lead a good language model to perform the task well. Don't be afraid to be creative.""")
        dataset_summary = og_dspy.InputField(desc="Summary of the dataset.")
        examples = og_dspy.InputField(format=dsp.passages2text, desc="Example(s) of the task")
        example_instructions = og_dspy.InputField(format=dsp.passages2text, desc="Example instruction(s) for this task.")
        tip = og_dspy.InputField(desc="A tip for something to keep in mind when generating the new instruction.")
...
class BasicGenerateInstructionAllFields(Signature):
    ...
    initial_prompt_template = og_dspy.InputField(desc="The initial prompt template for this task.")
    proposed_prompt_template = og_dspy.OutputField(desc="The improved prompt template for this task.")
    # proposed_prefix_for_output_field = og_dspy.OutputField(desc="The string at the end of the prompt, which will help the model start solving the task")
...
class BasicGenerateInstructionOnly(Signature):
    ...
    current_instruction = og_dspy.InputField(desc="The current instruction.")
    # examples_of_field_in_use = og_dspy.InputField(format=dsp.passages2text, desc="Examples of this field in use on examples from our task.")
    proposed_instruction = og_dspy.OutputField(desc="The proposed instruction (reply with the instruction only).")
...
class BasicGenerateField(Signature):
    ...
    current_field = og_dspy.InputField(desc="The current string in use for this field.")
    examples_of_field_in_use = og_dspy.InputField(format=dsp.passages2text, desc="Examples of this field in use on examples from our task.")
...
Your task is to propose a new instruction that will lead a good language model to perform the task even better. Don't be afraid to be creative."""
        ...
        # attempted_instructions = og_dspy.InputField(desc="Previously attempted task instructions, along with their resulting validation score, and an example of the instruction in use on a sample from our dataset.")
        proposed_instruction = og_dspy.OutputField(desc="The improved instructions for the language model")
        proposed_prefix_for_output_field = og_dspy.OutputField(desc="The string at the end of the prompt, which will help the model start solving the task")
